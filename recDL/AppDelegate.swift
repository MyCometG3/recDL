...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    public func handleStartRecording(_ notification: Notification) {
        ...
        // Keep scripting behavior synchronous.
    ...
    @IBAction func toggleRecording(_ sender: AnyObject) {
        ...
        if modifier(.option) {
            if manager != nil && (cachedRecordingState || recordingStartInProgress) {
            ...
            if recordingButton.state == NSControl.StateValue.on {
                ...
                Task { @MainActor [weak self] in
                    ...
                    await self.startRecordingNonBlocking(for: 0)
                }
            } else {
                // Stop recording
                Task { @MainActor [weak self] in
    ...
    @IBAction func actionStartRecording(_ sender: AnyObject) {
        ...
        // Reject multiple request
        if manager != nil && (cachedRecordingState || recordingStartInProgress || window.attachedSheet != nil) {
            NSSound.beep()
            return
        }
        ...
        panel.beginSheetModal(for: window, completionHandler: {[unowned self] result in
            ...
            if result == NSApplication.ModalResponse.OK {
                ...
                Task { @MainActor [weak self] in
                    ...
                    await self.startRecordingNonBlocking(for: min * 60)
                }
            }
        })
    }
    ...
    nonisolated static func printNilSelfWarning(_ function: String) {
        print("WARNING:AppDelegate: \(function) - self is nil")
    }
